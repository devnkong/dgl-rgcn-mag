...
def prepare_data(args):
    dataset = DglNodePropPredDataset(name="ogbn-mag")  # root='../datasets/dataset'
...
def train(g, model, node_embed, optimizer, train_loader, split_idx,  
    ...
    for epoch in range(args['n_epochs']):
        ...
        for input_nodes, seeds, blocks in train_loader:
            ...
            ####
            y_hat_l.append(logits)
            lbl_l.append(lbl)
        
        pbar.close()
        loss = total_loss / N_train
    
        correct_count = th.cat(y_hat_l, dim=0).argmax(dim=-1).cpu().eq(th.cat(lbl_l).cpu()).sum().item()
        print(f'########Train acc:{correct_count/th.cat(lbl_l).shape[0]}')
...
def test(g, model, node_embed, y_true, device, split_idx, args):
    ...
    def compute_acc(y_pred, y_true, split) :
        return y_pred[split].eq(y_true[split]).sum().item()/len(split)
    
    train_acc = my_train_acc = compute_acc(y_pred.squeeze(), lbl_l, split_idx['train']['paper'])
    valid_acc = my_val_acc = compute_acc(y_pred.squeeze(), lbl_l, split_idx['valid']['paper'])
    test_acc = my_test_acc = compute_acc(y_pred.squeeze(), lbl_l, split_idx['test']['paper'])
    # print(f'My Train: {my_train_acc}, My Val:{my_val_acc}, My Test:{my_test_acc}')
...
def main(args):
    ...
    device = f'cuda:1' if th.cuda.is_available() else 'cpu'
